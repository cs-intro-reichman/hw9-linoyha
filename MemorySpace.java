...
public class MemorySpace {
	...
	public int malloc(int length) {
        ...
        while (current != null) {
            if (current.block.length >= length) {
                int baseadress1 = current.block.baseAddress;
                MemoryBlock new1 = new MemoryBlock(baseadress1, length);
                allocatedList.addLast(new1);
	...
	public void defrag() {
		for (int i = 0; i < this.freeList.getSize(); i++) {
			MemoryBlock currentBlock = this.freeList.getBlock(i);
			int expectedBaseAddress = currentBlock.baseAddress + currentBlock.length;
